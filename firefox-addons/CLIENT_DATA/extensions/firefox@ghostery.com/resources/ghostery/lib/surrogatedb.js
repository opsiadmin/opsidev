...
function init() {
	var data = JSON.parse(utils.syncGet('databases/surrogates2.json'));
...
function getForTracker(script_src, app_id, pattern_id, parent_host_name) {
	var candidates = [];
	...
	return _.filter(candidates, function (surrogate) {
		...
		if (surrogate.hasOwnProperty("sites")) { // array of site hosts
			if (surrogate.sites.indexOf(parent_host_name) == -1) {
				return false;
			}
		}
...
exports.getForTracker = getForTracker;
exports.getForSite = getForSite;
