	...
	[censusCache, preCensusCache].forEach(function (cache, i) {
		for (var id in cache) {
			if (cache.hasOwnProperty(id)) {
				if (id != today) {
					utils.log("Cleaned up ", (i == 1 ? 'preCensusCache': 'censusCache'));
					delete cache[id];
				}
			}
		}
...
function recordPageInfo(domain, page_latency, page_timestamp, ad_spots) {
	if (!conf.ghostrank) {
		return;
	}
...
function recordStats(bug_url, deets) {
	...
	census_url = 'https://l.ghostery.com/api/census' +
		...
		'&cv=2' +
		'&ua=' + (MOBILE_MODE ? 'firefox-android' : 'firefox') +
		'&pt=' + deets.page_timestamp +
		'&tt=' + deets.tracker_timestamp +
