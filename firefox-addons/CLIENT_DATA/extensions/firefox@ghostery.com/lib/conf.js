...
function getDefaultLanguage() {
	var lang = i18n.getLanguage().split('-');
	if (lang.length > 1) {
...
var conf = (function loadConf() {
	// Load simple-storage backup in case it crashed
	// TODO only do this when we know store.json is missing/empty
	utils.loadStorageBackup();
		...
		defaults = {
			selected_cat_ids: {
				init: function () {
					...
					if (prefs('selected_cat_ids') !== undefined) {
						ids = prefs('selected_cat_ids');
					} else {
						prefs('selected_cat_ids', {});
					}
				...
				save: function (v) {
					prefs('selected_cat_ids', v);
					// dispatcher.trigger('conf.save.selected_cat_ids', _.clone(v));
				}
			},
			...
			unselected_app_ids: {
				init: function () {
					var ids = {};
					// legacy variable name: it actually stores app IDs
					if (prefs('unselected_app_ids') !== undefined) {
						ids = prefs('unselected_app_ids');
					} else {
						prefs('unselected_app_ids', {});
					}
				...
				save: function (v) {
					prefs('unselected_app_ids', v);
					dispatcher.trigger('conf.save.unselected_app_ids', _.clone(v));
				}
			},
			...
			site_blacklist: {
				init: function () {
					var blacklist = [];
					if (prefs('site_blacklist') !== undefined) {
						try {
							blacklist = prefs('site_blacklist');
						} catch (e) {}
					} else {
						prefs('site_blacklist', blacklist);
					}
				...
				save: function (v) {
					prefs('site_blacklist', v);
					dispatcher.trigger('conf.save.site_blacklist');
				}
			},
			language: {
				init: function () {
			...
			site_specific_cat_unblocks: {
				init: function () {
					var selections = {};
					if (prefs('site_specific_cat_unblocks') !== undefined) {
						try {
							selections = prefs('site_specific_cat_unblocks');
						} catch (e) {}
					} else {
						prefs('site_specific_cat_unblocks', selections);
					}
				...
				save: function (v) {
					prefs('site_specific_cat_unblocks', v);
				}
			},
			...
			site_specific_cat_blocks: {
				init: function () {
					var selections = {};
					if (prefs('site_specific_cat_blocks') !== undefined) {
						try {
							selections = prefs('site_specific_cat_blocks');
						} catch (e) {}
					} else {
						prefs('site_specific_cat_blocks', selections);
					}
				...
				save: function (v) {
					prefs('site_specific_cat_blocks', v);
				}
			},
			...
			site_specific_blocks: {
				init: function () {
					var selections = {};
					if (prefs('site_specific_blocks') !== undefined) {
						try {
							selections = prefs('site_specific_blocks');
						} catch (e) {}
					} else {
						prefs('site_specific_blocks', selections);
					}
			...
			alert_expanded: (prefs('alert_expanded') !== undefined ? prefs('alert_expanded') : false),
			...
			show_cmp: (prefs('show_cmp') !== undefined ? prefs('show_cmp') : true)
		};
	...
	_.each(defaults, function (sval, sname) {
		if (_.isObject(sval)) { // complex preference
			...
			var check_for_changes = _.debounce(function () {
				...
				//log(`checking old vs. new for ${sname}...`);

				if (!_.isEqual(v.current, v.old)) {
					//log(`change detected for ${sname}`);
